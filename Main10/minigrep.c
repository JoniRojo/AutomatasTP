...
void minigrep(Automata aut, char cadena[],int n);

int main(){
    int indiceCC = 0;
    char ER[20] = "a|b#";
    int bool = 1;
    char cadena[MAX3] = "ccccccabcc";
    ...
    printf("---------------------------------------------------\n");
    printf("AFND que reconoce si la cadena %s tiene una subcadena reconocida por la ER %s: \n", cadena, ER);
    minigrep(result,cadena,longitud);
}

void minigrep(Automata aut, char cadena[],int n){

    ListChar listaAlfabeto;
    insertarChar(&listaAlfabeto, '(');
    insertarChar(&listaAlfabeto, ')');
    ...
    insertarEnt(&listaEstados1, 76);
    ArregloEnt estados1 = listEntToArray(&listaEstados1);

    int inicial1 = 76;
    ListEnt listaFinales1;
    listaFinales1.head = NULL;
    insertarEnt(&listaFinales1,76);
    ArregloEnt finales1 = listEntToArray(&listaFinales1);

    Automata inic = crearAutomata( alfabeto, estados1, inicial1, finales1);
    anadirTransicion(&inic,76,'(',76);
    anadirTransicion(&inic,76,')',76);
    anadirTransicion(&inic,76,'a',76);
    anadirTransicion(&inic,76,'b',76);
    anadirTransicion(&inic,76,'c',76);

    Automata inicMasAut = concatenarAut(inic,aut);

    ListEnt listaEstados2;
    listaEstados2.head = NULL;
    insertarEnt(&listaEstados2, 86);
    ArregloEnt estados2 = listEntToArray(&listaEstados2);

    int inicial2 = 86;
    ListEnt listaFinales2;
    listaFinales2.head = NULL;
    insertarEnt(&listaFinales2,86);
    ArregloEnt finales2 = listEntToArray(&listaFinales2);

    Automata fin = crearAutomata( alfabeto, estados2, inicial2, finales2);

    anadirTransicion(&fin,86,'(',86);
    anadirTransicion(&fin,86,')',86);
    anadirTransicion(&fin,86,'a',86);
    anadirTransicion(&fin,86,'b',86);
    anadirTransicion(&fin,86,'c',86);

    Automata AutMasFin = concatenarAut(inicMasAut,fin);
    mostrarAutomata(AutMasFin);
    pertenece(AutMasFin,cadena,n);
}
